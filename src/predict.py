...
def load_model():
    ...
    """
    model_path = os.path.join(os.path.dirname(__file__), 'best_gbdt_model.pt')
    try:
        model = torch.load(model_path, map_location=torch.device("cpu"), weights_only=False)
        # Ensure the model has a 'predict' method or adjust accordingly.
        return model
...
def compute_logBCF(logKOW: float) -> float:
    ...
    Computes logBCF from logKOW using a piecewise formula:
      if logKOW < 1: logBCF = 0.15
      if 1 <= logKOW <= 6: logBCF = 0.85 * logKOW - 0.70
      if 6 < logKOW < 10: logBCF = -0.20 * (logKOW**2) + 2.74 * logKOW - 4.72
      if logKOW >= 10: logBCF = 2.68
    """
    if logKOW < 1:
        return 0.15
    elif logKOW <= 6:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Chemical Prediction Script')
    parser.add_argument('--mode', choices=['single', 'multiple'], default='single', help='Mode of prediction')
    parser.add_argument('--input', help='Path to CSV file for multiple predictions', default=None)
    args = parser.parse_args()

    model = load_model()
    # If model fails to load, output error and exit.
    if isinstance(model, dict) and 'error' in model:
        print(json.dumps(model))
        sys.exit(1)

    if args.mode == 'single':
        # Single mode: read JSON input from stdin.
        try:
            input_data = json.load(sys.stdin)
            cas = input_data.get('cas')
            logKOW = float(input_data.get('logKOW'))
        except Exception as e:
        ...
        try:
            ...
            # Assume the model has a 'predict' method; adjust this if necessary.
            prediction = model.predict(tensor_features)
        except Exception as e:
        ...
        # Multiple mode: process CSV file.
        try:
            df = pd.read_csv(args.input)
        except Exception as e:
            print(json.dumps({"error": "Error reading CSV file"}))
            sys.exit(1)
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                # Optionally, log errors or skip this chemical.
                continue
        
        if not combined_list:
            print(json.dumps({"error": "No valid chemicals processed"}))
            sys.exit(1)

        # Create DataFrame of features.
        features_df = pd.DataFrame(combined_list)
        non_numeric = ['CAS', 'SMILES']
        numeric_cols = [col for col in features_df.columns if col not in non_numeric]
        
        # Apply KNN imputation for NaN values with n_neighbors=5.
        from sklearn.impute import KNNImputer
        imputer = KNNImputer(n_neighbors=5)
        try:
            features_df[numeric_cols] = imputer.fit_transform(features_df[numeric_cols])
        except Exception as e:
            print(json.dumps({"error": "Error during imputation"}))
            sys.exit(1)

        # Predict for each chemical.
        for index, row in features_df.iterrows():
            try:
                feature_arr = prepare_numeric_features(row.to_dict())
                tensor_features = torch.tensor(feature_arr, dtype=torch.float)
                pred = model.predict(tensor_features)
                predictions.append(pred.tolist() if hasattr(pred, 'tolist') else pred)
            except Exception as e:
                predictions.append(None)

        features_df = features_df.copy()  # This helps eliminate fragmentation performance warnings.
        features_df['prediction'] = predictions

        output = {
            'predictions': predictions,
            'dataset': features_df.to_dict(orient='records')
        }
        print(json.dumps(output))
